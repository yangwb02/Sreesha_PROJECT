...
close all;
dataFolder = uigetdir();
dataDir    = dir([dataFolder '\' '*.dcm']);
feature=[];
featureVec = [];
for k = 1 : length(dataDir)
    I = dicomread([dataFolder '\' dataDir(k).name]);
...
%then is a noisy pixel. In this case, the P(X,Y) is replaced by the value of left neighborhood pixel value.
                smth(i,j) = smth(i,j-1);
            end
        end
    end
...
%%%%%---ROLLING BALL METHOD---%%%%%%
%%For instance the Rolling-ball algorithm uses a ball as a structuring element 
%and performs the top-hat transform[2].
%REF:Internet
%se = strel('disk',12);
...
SVMstruct=svmtrain(feature, class);
